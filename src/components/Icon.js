...
const icons = {
  ...
  // --- Add other SVGs here if needed ---
};

const Icon = ({ name, className = '' }) => {
    const SvgIcon = icons[name];
    if (!SvgIcon) {
        // Only log a warning in the console during development
        if (process.env.NODE_ENV === 'development') {
             console.warn(`Icon "${name}" not found.`);
        }
        return null; // Render nothing if icon not found
    }
    // Clone the SVG element to add the custom className
    return React.cloneElement(SvgIcon, {
        className: `${SvgIcon.props.className || ''} ${className}`.trim() // Ensure props.className exists
    });
};
